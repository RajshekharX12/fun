...
def db_get_game(gid: str) -> Optional[tuple]:
    ...
    # Ensure all 7 columns are selected: gid,chat_id,message_id,user_id,username,board,turn,status
...
def db_upsert_game(gid: str, chat_id: int, message_id: int, user_id: int, username: str, board: str, turn: str, status: str):
    ...
    # Note the 10 parameters (9 in VALUES, 1 for the second last_update in ON CONFLICT)
    db_exec("""
...
def attitude_message(status: str, username: str) -> str:
    ...
    # Use the username as provided, or fall back to a generic term
...
async def on_bot_start(cq: CallbackQuery, bot: Bot):
    ...
    # Check for game validity and user authority
    if not valid_game_click(cq, game):
        return
        
    # Unpack the 8 columns from the DB row
...
async def on_move(cq: CallbackQuery, bot: Bot):
    ...
    asyncio.create_task(
        bot_move_task(bot, gid, chat_id, message_id, user_id, username, board_after_human)
    )

async def bot_move_task(bot: Bot, gid: str, chat_id: int, message_id: int, user_id: int, username: str, board: str):
...
def valid_game_click(cq: CallbackQuery, game_row: Optional[tuple]) -> bool:
    ...
    # Note: We don't unpack all columns here, only the necessary ones for validation
...
async def edit_board(bot: Bot, chat_id: int, message_id: int, gid: str, user_id: int, username: str, board: str, status: str):
    ...
    try:
        ...
        # Handle "Message is not modified" quietly
        ...
        try:
            ...
            # The 'turn' parameter is set by the logic in the caller functions
            is_human_turn = HUMAN_turn(board)
            next_turn = HUMAN if is_human_turn else BOT
...
async def periodic_cleanup(bot: Bot):
    ...
    while True:
        ...
        # Get games that haven't been updated in 5 minutes
        expired_games = db_get_expired_games(MESSAGE_AUTODELETE_TIMEOUT)
        
        for gid, chat_id, message_id in expired_games:
            # Schedule the deletion task immediately (with timeout=0 inside the task)
...
async def main():
    ...
    # This will now attempt to fix the schema if columns are missing
    db_init() 
