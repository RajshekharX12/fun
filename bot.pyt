...
# - Added attitude and automatic message cleanup.
    ...
    try:
        with closing(sqlite3.connect(DB_PATH)) as con, con:
            ...
            CREATE TABLE IF NOT EXISTS games (
                ...
                username TEXT,                 -- Added for personalized attitude
                board TEXT NOT NULL,           -- 9 chars: ' ', 'X', 'O'
                turn TEXT NOT NULL,            -- 'X' or 'O'
                status TEXT NOT NULL,          -- 'PLAY', 'XWIN', 'OWIN', 'DRAW'
                last_update INTEGER NOT NULL   -- Added for auto-cleanup
            )
            """)
...
# Updated to include username and last_update
def db_get_game(gid: str) -> Optional[tuple]:
    """Retrieves a game by its ID."""
    rows = db_exec("SELECT gid,chat_id,message_id,user_id,username,board,turn,status FROM games WHERE gid=?", (gid,), fetch_one=True)
    return rows
...
def best_move(board: str) -> int:
    ...
    # Prioritize moves for faster Minimax calculation, without changing optimality
    ...
    for i in ordered_cells:
        ...
        # This order ensures the Minimax's natural tie-breaking (by index/priority) is used.
        if score > best_score:
            best_score = score
            best_idx = i
            
    if best_idx == -1:
        # This fallback is for safety and should not be reached in a non-full board
    ...
    for r in range(3):
        ...
        for c in range(3):
            ...
            if can_play and ch == EMPTY:
                ...
                # Keep it as 'noop' but specific to the game ID for group safety
...
async def on_newgame_cmd(m: Message, bot: Bot):
    ...
    # Ensure to pass the username for storage
    username = m.from_user.username or m.from_user.first_name
...
async def start_new_game(chat_id: int, user_id: int, username: str, bot: Bot):
    ...
    try:
        ...
        # Store message_id for later edits, including username
...
async def on_move(cq: CallbackQuery, bot: Bot):
    ...
    # This prevents the callback from blocking and guarantees the bot's turn
    asyncio.create_task(
        bot_move_task(bot, gid, chat_id, message_id, user_id, username, board_after_human, status_after_human)
    )

async def bot_move_task(bot: Bot, gid: str, chat_id: int, message_id: int, user_id: int, username: str, board: str, status: str):
    ...
    try:
        ...
        if status_after_bot != "PLAY":
            # Schedule message deletion after game ends
            asyncio.create_task(delete_message_after_timeout(bot, chat_id, message_id, gid, MESSAGE_AUTODELETE_TIMEOUT))
...
async def edit_board(bot: Bot, chat_id: int, message_id: int, gid: str, user_id: int, username: str, board: str, status: str):
    ...
    try:
        ...
        # aiogram raises "Message is not modified" if text/keyboard are identical, which is fine
        ...
        try:
            ...
            # The 'turn' parameter is handled by the caller logic
            db_upsert_game(gid, chat_id, msg.message_id, user_id, username, board, HUMAN if status == "PLAY" else BOT, status)
...
async def delete_message_after_timeout(bot: Bot, chat_id: int, message_id: int, gid: str, timeout: int):
    ...
    try:
        # Check if the message is the latest one for this game (optional, but safer)
        # We delete the message regardless, but the DB record is key
        await bot.delete_message(chat_id, message_id)
        logger.info(f"Deleted expired message {message_id} for game {gid}")
    except Exception as e:
        # Message might have already been deleted by a user or another process
        logger.debug(f"Could not delete message {message_id} in chat {chat_id}: {e}")
    finally:
        # Always delete the DB record to keep it clean
        db_delete_game(gid)
...
async def periodic_cleanup(bot: Bot):
    ...
    while True:
        ...
        # Get games that haven't been updated in 5 minutes (300 seconds)
        expired_games = db_get_expired_games(MESSAGE_AUTODELETE_TIMEOUT)
        
        for gid, chat_id, message_id in expired_games:
            # Schedule the deletion task immediately without waiting
